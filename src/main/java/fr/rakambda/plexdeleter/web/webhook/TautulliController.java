...
public class TautulliController{
	...
	private void updateRequirement(@NotNull TautulliWebhook data) throws RequestFailedException, IOException{
		if(Objects.isNull(data.getUserId())){
			log.warn("Not updating any requirement, couldn't not determine user id from {}", data);
			return;
		}
		...
		if(Objects.isNull(ratingKey)){
			log.warn("Not updating any requirement, couldn't not determine rating key from {}", data);
			return;
		}
		
		var userPersonEntity = userPersonRepository.findByPlexId(data.getUserId());
		if(userPersonEntity.isEmpty()){
			log.warn("Not updating any requirement, couldn't not find user person from {}", data);
			return;
		}
		
		var mediaEntity = mediaRepository.findByPlexId(ratingKey);
		if(mediaEntity.isEmpty()){
			log.warn("Not updating any requirement, couldn't not find media from {}", data);
			return;
		}
		...
		if(mediaRequirementEntity.isEmpty()){
			log.warn("Not updating any requirement, couldn't not find requirement with id {} from {}", requirementId, data);
			return;
		}
	...
	private void updateMedia(TautulliWebhook data) throws RequestFailedException, UpdateException{
		...
		if(Objects.isNull(ratingKey)){
			log.warn("Not updating any media, couldn't not determine rating key from {}", data);
			return;
		}
		
		var mediaEntity = mediaRepository.findByPlexId(ratingKey);
		if(mediaEntity.isEmpty()){
			log.warn("Not updating any media, couldn't not find media from {}", data);
			return;
		}
